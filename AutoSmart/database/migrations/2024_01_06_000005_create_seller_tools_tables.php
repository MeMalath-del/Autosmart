...
return new class extends Migration
{
    ...
    {
        // عمليات الاستيراد
        if (!Schema::hasTable('')) { Schema::create('product_imports', function (Blueprint $table) {
            $table->id();
        ...
        // سجل الاستيراد التفصيلي
        if (!Schema::hasTable('')) { Schema::create('import_logs', function (Blueprint $table) {
            $table->id();
        ...
        // ربط المتاجر الخارجية
        if (!Schema::hasTable('')) { Schema::create('external_store_connections', function (Blueprint $table) {
            $table->id();
            $table->foreignId('store_id')->constrained()->onDelete('cascade');
            $table->string('platform'); // amazon, ebay, noon, etc
        ...
        // منتجات مزامنة
        if (!Schema::hasTable('')) { Schema::create('synced_products', function (Blueprint $table) {
            $table->id();
        ...
        // العروض الموسمية
        if (!Schema::hasTable('')) { Schema::create('seasonal_campaigns', function (Blueprint $table) {
            $table->id();
        ...
        // تحليل المنافسين
        if (!Schema::hasTable('')) { Schema::create('competitor_prices', function (Blueprint $table) {
            $table->id();
            $table->foreignId('product_id')->constrained()->onDelete('cascade');
        ...
        // تقارير الأداء التنافسي
        if (!Schema::hasTable('')) { Schema::create('competitive_reports', function (Blueprint $table) {
            $table->id();
            $table->foreignId('store_id')->constrained()->onDelete('cascade');
            $table->date('report_date');
            $table->decimal('avg_price_vs_market', 5, 2)->nullable(); // -10% means 10% cheaper
