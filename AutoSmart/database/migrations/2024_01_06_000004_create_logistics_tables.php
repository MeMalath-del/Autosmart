...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!Schema::hasTable('')) { Schema::create('express_delivery_zones', function (Blueprint $table) {
            $table->id();
            $table->string('name');
            $table->string('city');
            $table->json('districts')->nullable();
            $table->decimal('same_day_fee', 10, 2);
            $table->decimal('express_fee', 10, 2); // 2-4 hours
            $table->time('cutoff_time')->default('14:00'); // آخر وقت للطلب
            $table->boolean('is_active')->default(true);
            $table->timestamps();
        }); }

        // طلبات التوصيل السريع
        if (!Schema::hasTable('')) { Schema::create('express_deliveries', function (Blueprint $table) {
            $table->id();
        ...
        // خزائن الاستلام الذكية
        if (!Schema::hasTable('')) { Schema::create('smart_lockers', function (Blueprint $table) {
            $table->id();
            $table->string('name');
            ...
            $table->json('compartment_sizes')->nullable(); // small, medium, large counts
        ...
        // حجوزات الخزائن
        if (!Schema::hasTable('')) { Schema::create('locker_reservations', function (Blueprint $table) {
            $table->id();
            $table->foreignId('order_id')->constrained()->onDelete('cascade');
            $table->foreignId('locker_id')->constrained('smart_lockers')->onDelete('cascade');
            $table->string('compartment_number');
            $table->string('compartment_size'); // small, medium, large
        ...
        // تجميع الطلبات
        if (!Schema::hasTable('')) { Schema::create('order_bundles', function (Blueprint $table) {
            $table->id();
            ...
            $table->datetime('bundle_deadline'); // آخر موعد للإضافة
            $table->enum('status', ['open', 'closed', 'shipped'])->default('open');
            $table->timestamps();
        }); }

        // طلبات مجمعة
        if (!Schema::hasTable('')) { Schema::create('bundled_orders', function (Blueprint $table) {
            $table->id();
        ...
        // دول الشحن الدولي
        if (!Schema::hasTable('')) { Schema::create('international_shipping_zones', function (Blueprint $table) {
            $table->id();
            $table->string('country_code', 2);
            $table->string('country_name');
            $table->string('country_name_ar')->nullable();
            $table->string('zone'); // GCC, Middle East, etc
        ...
        // طلبات الشحن الدولي
        if (!Schema::hasTable('')) { Schema::create('international_shipments', function (Blueprint $table) {
            $table->id();
            $table->foreignId('order_id')->constrained()->onDelete('cascade');
