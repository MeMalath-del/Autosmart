...
class PartLifecycle extends Model
{
    ...
    {
        if ($this->expected_replacement_date && $this->expected_replacement_date->diffInDays(now()) <= 30) {
            return true;
        }
    ...
    {
        if ($this->actual_replacement_date) {
        ...
        if ($this->expected_replacement_date?->isPast()) {
