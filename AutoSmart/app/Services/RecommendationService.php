...
class RecommendationService
{
        ...
        return Product::where('id', '!=', $product->id)
            ...
            ->where(function ($q) use ($product) {
                $q->where('category_id', $product->category_id)
                    ->orWhereHas('carModels', fn ($q2) => $q2->whereIn('car_model_id', $product->carModels->pluck('id'))
                    );
            })
            ->inRandomOrder()
            ->take($limit)
    ...
    public function getFrequentlyBoughtTogether(Product $product, int $limit = 4): Collection
    {
        ...
        return Product::whereHas('orderItems', fn ($q) => $q->whereIn('order_id', $orderIds)->where('product_id', '!=', $product->id)
        )
            ->where('is_active', true)
