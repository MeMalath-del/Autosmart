...
class DynamicPricingService
{
    ...
    protected function evaluateConditions(DynamicPricingRule $rule, Product $product): bool
    {
        ...
        if ($rule->type === 'inventory') {
            ...
            if ($product->quantity <= $lowStockThreshold) return true;
            if ($product->quantity >= $highStockThreshold) return true;
        }
        ...
        if ($rule->type === 'time') {
            ...
            if (!empty($hours) && !in_array(now()->hour, $hours)) return false;
            if (!empty($days) && !in_array(now()->dayOfWeek, $days)) return false;
            return true;
        }
